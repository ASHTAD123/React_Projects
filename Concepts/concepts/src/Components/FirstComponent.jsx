...
function FirstComponent(){
    ...
    // item among its siblings. Usually, a key should be coming from your data,
    //  such as a database ID. React uses your keys to know what happened if you later insert, delete, or reorder the items
    const listOfNames = names.map( item =>   
                        
                        <li
                         key={item.id}
                         style={{color: item.age==25 ? 'red' : 'blue' }}>
                           {item.name}
                     </li>
    );
